...
void		draw_column(t_all *node, t_player plr, t_help3 help3)
{
	...
	while (help.x < help.max_x)
		my_mlx_pixel_put(node, node->mapa->l, help.x++, create_trgb(node->ceiling->r, node->ceiling->g, node->ceiling->b));
	...
	while (help.y < node->res_y)
		my_mlx_pixel_put(node, node->mapa->l, help.y++, create_trgb(node->floor->r, node->floor->g, node->floor->b));
}
